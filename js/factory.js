...
angular.module('search',[]).factory('Search',function(){
	var cart = {
		...
		addLocation: function(outlet) {
			localStorage.setItem("search",JSON.stringify(outlet));
		},
		remove: function(){
			localStorage.setItem("search",JSON.stringify([]));
			localStorage.setItem("latitude","");
...
angular.module('customer',[]).factory('Customer',function($rootScope,$http){
	var cart = {
		...
		refreshAddress: function() {
			customer = JSON.parse(localStorage.getItem("customer"));
			var urlLogin = url + "/getAddresDetail.php?customer_id="+customer.customer_id+"&callback=JSON_CALLBACK";
